...
4. **설명은 간결해야 함. 코드 스니펫을 붙여넣지 말 것** — 리뷰어는 diff를 직접 읽을 수 있으므로 본문에 코드 블록, 함수 시그니처, 파일 경로 나열을 넣지 않음
5. **디자인 의도(design intent)와 동작 변경(behavior changes)에 집중** — "어떻게 구현했는지"가 아니라 "왜 했고 무엇이 달라지는지"를 서술
6. **"작업 내용" 섹션은 최대 3-5개의 불릿으로 제한** — 변경이 많으면 관련 항목끼리 묶어서 요약. 각 불릿은 한 줄로 핵심만
7. 비즈니스 맥락에 대한 정보가 필요한 경우 사용자에게 질문하여 확인
8. 테스트 방법이나 체크리스트가 템플릿에 있다면 해당 항목도 작성
