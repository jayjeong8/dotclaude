...
3. **리뷰어 유형 구분**: 사람 리뷰와 AI 리뷰(Coderabbit 등)를 분리하여 처리
   - AI 리뷰어 식별: `coderabbitai[bot]`, `github-actions[bot]` 등 `[bot]` 접미사
   - 사람 리뷰 → 기존 분류 기준 적용
   - AI 리뷰 → 아래 Coderabbit 파싱 규칙 적용
...
5. **Coderabbit AI 리뷰 파싱**:
   - `actionable` 태그/섹션 → **수정 예정** (구체적 코드 변경 제안이 있는 경우)
   - `suggestion` 태그/섹션 → **검토 필요** (사용자 판단 대기)
   - `nitpick` 태그/섹션 → **선택적** (사용자가 원하면 반영)
   - `praise` / `informational` → **무시**

6. **AI 리뷰 오탐 자동 식별** (다음 조건에 해당하면 "수정하지 않을 항목"으로 분류):
   - 의도된 패턴: 코드베이스에서 동일 패턴이 3회 이상 사용됨
   - 이미 해결됨: 리뷰 시점 이후 커밋에서 해당 코드가 변경됨
   - 프레임워크 관례: 프레임워크/라이브러리가 권장하는 패턴
   - 컨텍스트 부재: AI가 프로젝트 컨벤션을 이해하지 못한 경우
...
**Coderabbit 답글 형식:**
- 반영한 경우: `반영했습니다. (커밋해시)\n- 변경 내용 요약`
- 오탐인 경우: `의도된 패턴입니다.\n- 프로젝트에서 동일 패턴이 N회 사용됩니다. (예: file1.ts:L10, file2.ts:L20)`
- 선택적 미반영: `프로젝트 컨벤션에 따라 현재 방식을 유지합니다.\n- 이유 설명`
